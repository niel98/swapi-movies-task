...
const checkDuplicateRegistration = asyncHandler(async (req, res, next) => {
  // Email
  db.DispatchCompany.findOne({
    where: {
    ...
    if (user) {
      res.status(400).send({
        status: "fail",
        message: "Company with this registration number exists",
      });
      return;
    }
